...
import com.f5704.ask.web.model.FAgg;


public class FAggTopResultExtractor extends FResultMapper{
	
	private String term  , name ;
	
	public FAggTopResultExtractor(String term , String name){
		this.term = term ;
		this.name = name ;
	}
			...
			for (int i = pageable.getPageNumber()*pageable.getPageSize();i<agg.getBuckets().size()  ; i++) {
				...
				if(!StringUtils.isBlank(name) && entry.getAggregations().get(name)!=null){
					...
					for (SearchHit hit : topHits.getHits().getHits()) {
						...
						if(data instanceof FAgg){
							((FAgg) data).setRowcount((int) topHits.getHits().getTotalHits());
						}
						results.add(data) ;
					}
				}
