...
@Component
public class FFuRepositoryImpl implements FFuEsCommonRepository{
	...
	public void setElasticsearchTemplate(ElasticsearchTemplate elasticsearchTemplate) {
        ...
        if(!elasticsearchTemplate.typeExists("f5704" , "f5704_ask_message")){
	...
	public Page<Serializable> findByCon(Class clazz,NativeSearchQueryBuilder searchQueryBuilder,  String q , final int p , final int ps) {
	    ...
	    if(elasticsearchTemplate.indexExists(clazz)){
	    	if(!StringUtils.isBlank(q)){
	    		pages = elasticsearchTemplate.queryForPage(searchQuery, clazz, new FResultMapper());
	    	}else{
	    		pages = elasticsearchTemplate.queryForPage(searchQuery, clazz , new FAggTopResultExtractor("userid" , "top"));
	    	}
	    }
	    return pages ; 
	}
