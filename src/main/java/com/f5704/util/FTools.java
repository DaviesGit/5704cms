...
import com.f5704.util.event.MultiUpdateEvent;
...
public class FTools {
	...
	public static void published(UserEvent event){
		...
		Disruptor<UserDataEvent> disruptor = (Disruptor<UserDataEvent>) FDataContext.getContext().getBean("disruptor") ;
	...
	public static void multiupdate(MultiUpdateEvent event){
		Disruptor<UserDataEvent> disruptor = (Disruptor<UserDataEvent>) FDataContext.getContext().getBean("multiupdate") ;
		long seq = disruptor.getRingBuffer().next();
		disruptor.getRingBuffer().get(seq).setEvent(event); ;
		disruptor.getRingBuffer().publish(seq);
	}
		...
		if(values!=null && values.size()>0){
			...
			for(int i= 0 ; i<values.size() ; i++){
				...
				if(value.length>=2){
					...
					if(FDataContext.OnlineUserInviteStatus.DEFAULT.toString().equals(invitestatus) || invitestatus == null){
						report.setUsers((long) value[1]);
					}else if(FDataContext.OnlineUserInviteStatus.INVITE.toString().equals(invitestatus)){
						report.setInviteusers((long) value[1]);
					}else if(FDataContext.OnlineUserInviteStatus.REFUSE.toString().equals(invitestatus)){
		...
		if(values!=null && values.size()>0){
			...
			for(int i= 0 ; i<values.size() ; i++){
				...
				if(value.length>=2){
					...
					if(FDataContext.OnlineUserInviteStatus.DEFAULT.toString().equals(invitestatus) || invitestatus == null){
						report.setUsers((long) value[1]);
					}else if(FDataContext.OnlineUserInviteStatus.ACCEPT.toString().equals(invitestatus)){
						report.setInviteusers((long) value[1]);
					}else if(FDataContext.OnlineUserInviteStatus.REFUSE.toString().equals(invitestatus)){
						report.setRefuseusers((long) value[1]);
					}
		...
		if(values!=null && values.size()>0){
			for(int i= 0 ; i<values.size() ; i++){
				...
				if(value.length>=2){
					...
					if(FDataContext.WeiXinEventTypeEnum.SUB.toString().equals(event)){
						report.setSubs((long) value[1]);
					}else if(FDataContext.WeiXinEventTypeEnum.UNSUB.toString().equals(event)){
    ...
    public static String encryption(String str) throws NoSuchAlgorithmException{
    	...
    	textEncryptor.setPassword(FDataContext.getSystemSecrityPassword());
    ...
    public static String filterChatMessage(String message){
    	...
    	for (Element element : pngs) {
    		...
    		if(imgUrl.indexOf("/res/image") >= 0){
    			element.attr("class", "f5704-media-image") ;
    		}
    	}
