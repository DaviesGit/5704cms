 ...
 */
public class FTableMetaData {

	private final String catalog;
	private final String schema;
	private String name;
	private final List<FColumnMetadata> columnMetaData = new ArrayList<FColumnMetadata>();
	 ...
	 * @throws SQLException
	 */
	public FTableMetaData(ResultSet rs, DatabaseMetaData meta, boolean extras, boolean upcase , boolean loadColumns , String dbtype)
			throws SQLException {
	 ...
	 * @throws SQLException
	 */
	public FTableMetaData(String tableName , String tableCatalog , String tableSchema, ResultSetMetaData meta, boolean extras)
			throws SQLException {
	...
	public List<FColumnMetadata> getColumnMetadatas() {
	...
	public void addColumn(ResultSet rs , boolean upcase ) throws SQLException {
		...
		if (columName.get(column) == null) {
			FColumnMetadata info = new FColumnMetadata(rs , upcase);
	...
	public void addSqlColumn(String name , String typeName , int typeCode , int columSize) throws SQLException {
		...
		if (columName.get(name) == null) {
			FColumnMetadata info = new FColumnMetadata(name , typeName , typeCode , columSize);
			columnMetaData.add(info) ;
			columName.put(info.getName().toLowerCase(),"");
		}
	}
